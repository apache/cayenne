...
The *cayenne-maven-plugin* is used. Among other things, it allows to reverse-engineer the Cayenne model from the database and to update the model after the database has been changed.

==== CayenneModeler

CayenneModeler is the GUI tool used in this tutorial to inspect and edit the model. Download the appropriate Cayenne distribution from the Cayenne web site and launch the Modeler from it (e.g. `java -jar CayenneModeler.jar`, or open `CayenneModeler.app` on macOS).
