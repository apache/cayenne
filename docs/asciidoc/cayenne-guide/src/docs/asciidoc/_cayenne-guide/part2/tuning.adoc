...
ObjectSelect<Artist> query = ObjectSelect
    ...
    // Equivalent to ".prefetch("paintings", PrefetchTreeNode.DISJOINT_PREFETCH_SEMANTICS)")
...
Prefetch semantics defines a strategy to prefetch relationships. Depending on semantics, Cayenne would generate
...
enough information to Cayenne to build dependent prefetch queries. So "DISJOINT" will be quietly ignored. "JOINT"
