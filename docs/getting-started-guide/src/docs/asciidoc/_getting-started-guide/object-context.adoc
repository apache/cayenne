...
use it to obtain an instance of an `ObjectContext`. The runtime needs to know which database to connect to.
The database connection is not a part of the mapping project. It is defined in the code as a `DataSource`.
Here we are using an in-memory Derby database, so when you stop the application, all the data will be lost.
In most real-life cases you'll be connecting to a database that actually persists its data on
disk, but an in-memory DB will do for the simple tutorial. We are wrapping the `DataSource` in a
`DataNodeDescriptor` with `createSchemaIfNeeded()`, so that Cayenne creates a new schema on Derby based on
the ORM mapping when the application starts.
...
In the console you'll see output similar to this, indicating that Cayenne stack has been started:
    ...
    INFO: setting DataNode 'tutorial' as default, used by all unlinked DataMaps
