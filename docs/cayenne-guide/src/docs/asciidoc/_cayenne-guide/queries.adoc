...
Query objects are used to communicate with the database. Most often, to select some objects matching a criteria.
Cayenne knows how to translate queries to SQL statements appropriate for a particular database engine.
Other types of queries allow to run native SQL, call stored procedures, etc. When committing objects,
...
Object queries (most notably `ObjectSelect`) are built with abstractions originating in the object model (the "object"
side in the "object-relational" divide). E.g. `ObjectSelect` consists of a Java class of objects to fetch, a qualifier
expression, orderings, etc. - all of this expressed in terms of the object model.
...
include::queries/object-query-language.adoc[]
