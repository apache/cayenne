...
INSERT INTO table1(id, name) VALUES(?, ?) [bind:[id:3,name:'n3']..3..[id:2,name:'n2']] [updated:5]
----

A statement that produces more than one result (such as a stored procedure) reports the extra results on `also ...` continuation lines. The default implementation is `org.apache.cayenne.log.Slf4jSqlLogger`, which logs via the slf4j-api library under the fixed logger name `cayenne-sql`. Statement lines are logged at `INFO`; transaction boundaries (`tx started`, `tx committed`, `tx rolled back`) at `DEBUG`. To silence SQL logging entirely, set the `cayenne-sql` logger level above `INFO`.
