...
[source, java]
----
EJBQLQuery<Object[]> query = new EJBQLQuery<>("select a, COUNT(p) FROM Artist a JOIN a.paintings p GROUP BY a");
...
[source, java]
----
EJBQLQuery<String> query = new EJBQLQuery<>("select a.name FROM Artist a");
