...
List<QueryResultItem> result = ProcedureCall.query("my_procedure")
    .param("p1", "abc")
    .param("p2", 3000)
    .call(context);
----

`QueryResultItem` is a sealed interface with `Select`, `Update`, `Iterator` and `OutParameters` variants, listed in
...
The values of OUT and INOUT parameters are reported as a separate `QueryResultItem.OutParameters` item of the
result, a map keyed by parameter name:

[source, java]
----
for (QueryResultItem item : ProcedureCall.query("my_procedure").call(context)) {
    switch (item) {
        case QueryResultItem.Select<?> select -> process(select.objects());
        case QueryResultItem.Update update -> process(update.counts());
        case QueryResultItem.OutParameters out -> process(out.values().get("out_param"));
        default -> {}
    }
}
----
