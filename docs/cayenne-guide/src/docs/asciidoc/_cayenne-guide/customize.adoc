...
----
include::{java-examples}/di/Service1.java[tags=content]
----

[source,java,indent=0]
----
include::{java-examples}/di/Service2.java[tags=content]
----
...
[source,java,indent=0]
----
include::{java-examples}/di/Service1Impl.java[tags=content]
----
...
[source,java,indent=0]
----
include::{java-examples}/di/constructor/Service1Impl.java[tags=content]
----

[source,java,indent=0]
----
include::{java-examples}/di/Service2Impl.java[tags=content]
----

Now let's create a module implementing `org.apache.cayenne.di.Module` interface that will contain DI configuration. A module binds service objects to keys that are reference. Binder provided by container implements fluent API to connect the key to implementation, and to configure various binding options (the options, such as scope, are demonstrated later in this chapter). The simplest form of a key is a Java Class object representing service interface. Here is a module that binds Service1 and Service2 to corresponding default implementations:

[source,java,indent=0]
----
include::{java-examples}/di/Module1.java[tags=content]
----
...
[source,java,indent=0]
----
include::{java-examples}/CustomizeTest.java[tags=injector]
----
...
[source,java,indent=0]
----
include::{java-examples}/CustomizeTest.java[tags=getInstance]
----
...
[source]
----
S1 String: 0_Service1Impl
S1 String: 1_Service1Impl
S1 String: 2_Service1Impl
S1 String: 3_Service1Impl
S1 String: 4_Service1Impl
----
...
[source,java,indent=0]
----
include::{java-examples}/CustomizeTest.java[tags=bindings]
----
...
[source,java,indent=0]
----
include::{java-examples}/CustomizeTest.java[tags=withoutScope]
----
...
[source,java,indent=0]
----
include::{java-examples}/customize/MyExtensionsModule.java[tags=content]
----

[source,java,indent=0]
----
include::{java-examples}/CustomizeTest.java[tags=extensionsModule]
----
...
[source,java,indent=0]
----
include::{java-examples}/customize/MyDbAdapterDetector.java[tags=content]
----

[source,java,indent=0]
----
include::{java-examples}/CustomizeTest.java[tags=adapterDetector]
----
...
[source,java,indent=0]
----
include::{java-examples}/customize/Money.java[tags=content]
----
...
[source,java,indent=0]
----
include::{java-examples}/customize/MoneyValueObjectType.java[tags=content]
----
...
[source,java,indent=0]
----
include::{java-examples}/CustomizeTest.java[tags=valueObjectType]
----

More examples of implementation you can find in
https://github.com/apache/cayenne/blob/master/cayenne/src/main/java/org/apache/cayenne/access/types/PeriodValueType.java[cayenne core].
...
[source,java,indent=0]
----
include::{java-examples}/customize/DoubleArrayType.java[tags=content]
----

[source,java,indent=0]
----
include::{java-examples}/CustomizeTest.java[tags=extendedType]
----
...
`org.apache.cayenne.log.SQLLogger` is the service that defines the SQL logging API for Cayenne internals. It logs each executed statement as a single compact line combining the SQL, its bindings, the result count and the execution time, e.g. (long lines are wrapped here for readability):

----
SELECT id FROM my_table WHERE user_id = ? | bind:[user_id:15] selected:1 time_ms:2
INSERT INTO table1(id, name) VALUES(?, ?)
    | bind:[id:1,name:'n1'][id:2,name:'n2'] updated:2 time_ms:4
----

A statement that produces more than one result (such as a stored procedure) reports the extra results on `also ...` continuation lines. The default implementation is `org.apache.cayenne.log.Slf4jSQLLogger`, which logs via the slf4j-api library under the fixed logger name `cayenne-sql`. Both the statement lines and the transaction boundaries (`tx started`, `tx committed`, `tx rolled back`) are logged at `INFO`. To silence SQL logging entirely, set the `cayenne-sql` logger level above `INFO`.

Each row of a batch is logged in its own square brackets. Once the batch exceeds `cayenne.jdbc.log.batch.threshold` rows (default 20), the bindings are truncated to that number of rows, split between the head and the tail of the batch, with the count of the elided rows in the middle. E.g., this is a batch of 5 rows logged with the threshold of 3:

----
INSERT INTO table1(id, name) VALUES(?, ?)
    | bind:[id:1,name:'n1'][id:2,name:'n2']..2..[id:5,name:'n5'] updated:5 time_ms:4
----
