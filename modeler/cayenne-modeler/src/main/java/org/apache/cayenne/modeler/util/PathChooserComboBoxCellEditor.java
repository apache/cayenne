...
/**
 * This class used as cell editor , when you need
 * to choose path in comboBox and use autocompletion.
 */
public abstract class PathChooserComboBoxCellEditor extends AbstractCellEditor implements TableCellEditor {
    ...
    protected  void initializeCombo(CayenneTableModel model, int row, final JTable table){
        Object currentNode =
                getCurrentNodeToInitializeCombo(model,row);
        String dbAttributePath =
                getPathToInitializeCombo(model,row);
    ...
    protected void parsePathString(char lastEnteredCharacter){
        ...
        String pathString = ((JTextComponent) (comboBoxPathChooser).
