...
public class DbRelationshipUndoableEdit extends CayenneUndoableEdit {

    private final DbRelationship relationship;
    private final DbRelationship prevRelationship;

    public DbRelationshipUndoableEdit(ProjectSession session, DbRelationship relationship) {
        super(session);
    ...
    private void fireRelationshipEvent(DbRelationship relToFire, DbRelationship currRel) {
        DbEntity dbEntity = currRel.getSourceEntity();
        dbEntity.removeRelationship(currRel.getName());
        dbEntity.addRelationship(relToFire);
        session.fireDbRelationshipEvent(DbRelationshipEvent.ofAdd(this, relToFire, relToFire.getSourceEntity()));
    }
