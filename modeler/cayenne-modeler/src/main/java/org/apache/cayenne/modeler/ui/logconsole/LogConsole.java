...
public class LogConsole extends AppPanel implements LogAppender {
    ...
    public void toggle() {
        if (new LogConsolePrefs(app.getPrefsLocator().appNode(LogConsolePrefs.NODE)).toggleShowConsole()) {
            appear();
        } else {
            disappear();
        }
    ...
    public void showConsoleIfNeeded() {
        if (new LogConsolePrefs(app.getPrefsLocator().appNode(LogConsolePrefs.NODE)).isShowConsole()) {
            appear();
        }
    }
