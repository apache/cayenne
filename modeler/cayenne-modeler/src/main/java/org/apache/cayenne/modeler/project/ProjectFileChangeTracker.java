...
class ProjectFileChangeTracker extends Thread {
    ...
    public ProjectFileChangeTracker(ProjectSession session) {
    ...
    private void doOnRemove() {
        if (session.project() != null) {
            ...
            SwingUtilities.invokeLater(() -> {
                ...
                FileDeletedDialog dialog = new FileDeletedDialog(session.app().getFrame());
                dialog.show();

                if (dialog.shouldSave()) {
                    session.app().getActionManager().getAction(SaveAction.class).performAction(null);
                } else if (dialog.shouldClose()) {
                    session.app().getFrame().onProjectClosed();
                } else {
                    session.setDirty(true);
                }
    ...
    private boolean showConfirmation(String message) {
        int outcome = JOptionPane.showConfirmDialog(session.app().getFrame(), message, "File changed", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
