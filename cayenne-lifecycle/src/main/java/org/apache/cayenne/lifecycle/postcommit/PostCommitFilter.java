...
public class PostCommitFilter implements DataChannelFilter {
	...
	public GraphDiff onSync(ObjectContext originatingContext, GraphDiff beforeDiff, int syncType,
		...
		afterCommit(changes, beforeDiff, afterDiff);
	...
	private void afterCommit(MutableChangeMap changes, GraphDiff contextDiff, GraphDiff dbDiff) {

		GraphChangeHandler handler = new DiffFilter(entityFactory, new DiffProcessor(changes));
		contextDiff.apply(handler);
