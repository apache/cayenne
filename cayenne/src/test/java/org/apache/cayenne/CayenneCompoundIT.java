...
public class CayenneCompoundIT {
    ...
    private void createCompoundPKs(int size) throws Exception {
        for (int i = 0; i < size; i++) {
            tCompoundPKTest.insert("PK" + i, "PK" + (2 * i), String.format("BBB%02d", i));
        }
    }
    ...
    @Test
    public void paginatedObjectSelect() throws Exception {
        createCompoundPKs(20);

        int queriesCount = env.runWithQueryCounter(() -> {
            List<CompoundPkTestEntity> result = ObjectSelect.query(CompoundPkTestEntity.class)
                    .pageSize(7)
                    .orderBy(CompoundPkTestEntity.NAME.asc())
                    .select(env.context());
            assertEquals(20, result.size());

            int i = 0;
            for (CompoundPkTestEntity e : result) {
                assertEquals(PersistenceState.COMMITTED, e.getPersistenceState());
