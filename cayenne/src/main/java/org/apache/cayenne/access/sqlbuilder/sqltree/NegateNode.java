...
/**
 * Unary minus. Emits the {@code -} sign hugging its operand ({@code -1}), rather than treating it as a separated
 * token ({@code - 1}).
 *
 * @since 5.0
 */
public class NegateNode extends Node {
    ...
    public SQLAppendable append(SQLAppendable buffer, SQLGenerationContext context) {
        return buffer.appendTokenSeparator().append('-').suppressNextTokenSeparator();
    }
