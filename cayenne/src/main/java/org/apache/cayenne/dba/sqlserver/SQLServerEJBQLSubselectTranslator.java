...
class SQLServerEJBQLSubselectTranslator extends EJBQLSelectTranslator {

    boolean isExists;

    SQLServerEJBQLSubselectTranslator(EJBQLTranslationContext context, boolean isExists) {
        super(context);
    ...
    public boolean visitSelectExpressions(EJBQLExpression expression) {
        if (isExists) {
            ...
            super.visitSelectExpressions(expression);
        }
        return false;
    }
}
