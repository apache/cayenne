...
class SQLServerEJBQLConditionTranslator extends EJBQLConditionTranslator {

    boolean isExists;

    public SQLServerEJBQLConditionTranslator(EJBQLTranslationContext context) {
        super(context);
    }
    ...
    public boolean visitSubselect(EJBQLExpression expression) {
        context.onSubselect();
        context.append(" (");
        expression.visit(new SQLServerEJBQLSubselectTranslator(context, isExists));
        context.append(')');
        isExists = false;
        return false;
    }
