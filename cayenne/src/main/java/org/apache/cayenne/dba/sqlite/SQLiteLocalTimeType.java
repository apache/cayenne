...
/**
 * Stores {@link LocalTime} as text, see {@link SQLiteJavaTimeFormats}. Reads both a plain time and a full timestamp
 * (the form the driver stores for {@code java.util.Date}).
 *
 * @since 5.0
 */
class SQLiteLocalTimeType extends LocalTimeType {
    ...
    public void setJdbcObject(PreparedStatement statement, LocalTime value, int pos, int type, int scale)
        ...
        if (value == null) {
            ...
            statement.setString(pos, SQLiteJavaTimeFormats.TIME_OUT.format(value));
        }
    }
    ...
    private static LocalTime parse(String value) {
        ...
        return value.indexOf('-') > 0
                ? SQLiteJavaTimeFormats.parseTimestamp(value).toLocalTime()
