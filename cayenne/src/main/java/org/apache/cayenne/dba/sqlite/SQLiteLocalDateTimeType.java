...
/**
 * Stores {@link LocalDateTime} as text, see {@link SQLiteJavaTimeFormats}.
 *
 * @since 5.0
 */
class SQLiteLocalDateTimeType extends LocalDateTimeType {
    ...
    public void setJdbcObject(PreparedStatement statement, LocalDateTime value, int pos, int type, int scale)
        ...
        if (value == null) {
            ...
            statement.setString(pos, SQLiteJavaTimeFormats.TIMESTAMP_OUT.format(value));
        }
    }

    private static LocalDateTime parse(String value) {
        return value == null ? null : SQLiteJavaTimeFormats.parseTimestamp(value);
    }
}
